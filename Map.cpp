...
void Map::resetMap()
{
    ...
    player_position_[1] = 0; //the upper most left corner

    // set dungeon exit
    dungeon_exit_[0] = num_rows_ - 1;
    dungeon_exit_[1] = num_cols_ / 2; // at the middle column

    for (int i = 0; i < max_npcs_; i++)
    {
        npc_positions_[i][0] = -1;
        npc_positions_[i][1] = -1; // -1 means that NPCs have not been assigned to random spaces yet
    }
    npc_count_ = 0;
    ...
    for (int i = 0; i < max_rooms_; i++)
    {
        ...
        room_positions_[i][1] = -1; // -1 means that rooms have not been assigned to yet
    }
    room_count_ = 0;
    ...
    for (int i = 0; i < num_rows_; i++)
    {
        ...
        {
            map_data_[i][j] = UNEXPLORED; //basically setting every space of map to be " - "
        }
    }
    map_data_[dungeon_exit_[0]][dungeon_exit_[1]] = EXIT; // settign the exit space to be " E "
}

char Map::getMapdata(int row, int col) //added
{
...
int Map::getMaxNpcs() //additional function
{
    return max_npcs_;
}
int Map::getMaxRooms() // additional function
{
    return max_rooms_;
}
 ...
 * Return: boolean (bool)
 */
bool Map::isNPCLocation(int row, int col) // this one is important -> make sure understand how to implement and test it***
{
    if (!isOnMap(row, col))
    {
        return false;
    }
    for (int i = 0; i < npc_count_; i++) //going through each NPC's location and compare it to the current location.
    {
...
bool Map::isRoomLocation(int row, int col)
{
    ...
    {
        if (row == room_positions_[i][0] && col == room_positions_[i][1]) //going through each Room's location and compare it to the current location
        {
            return true;
        }
    }
...
bool Map::isExplored(int row, int col)
{
    ...
    if (map_data_[row][col] == EXPLORED) // the current location is " " (explored)
    {
        return true;
    }
    for (int i = 0; i < npc_count_; i++)
    {
        ...
        {
            if (npc_positions_[i][2] == true) // the [2] store information whether you have already talked to NPC or not (True = 1) -> will be shown on the map
            {
                return true; // if you have talked to NPC, you have explored the space already
            }
            break;
        }
    }
 ...
 * Return: boolean (bool)
 */
bool Map::addNPC(int row, int col) //this one is very important!!!!!
{
    if (npc_count_ >= max_npcs_) // npc_count_ keep increasing until reaching max_npcs_ (keep adding)
    {
    ...
    npc_positions_[npc_count_][2] = false; //have not been interacted with 
    //npc_positions_[npc_count_][2] = true; //this for just checking if they're populated in the map (passed)
    map_data_[row][col] = NPC; //* should not be shown unless player have entered
    npc_count_++;
    return true;
}
...
bool Map::move(char direction)
{
    ...
    // if new location is an NPC location, mark as explored  -> this is a little confusing isn't it supposed to be mark as NPC
...
// bool Map::move(string direction) //using string instead of char for the conveniece of 
// {
//     // check input char and move accordingly
//     switch (direction)
//     {
